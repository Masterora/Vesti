...
const exactZhMessages: Record<string, string> = {
  ...
  "Creator cannot claim their own contract": "甲方不能认领自己的项目。",
  "Only public contracts can be claimed": "只有公开项目才可以被认领。",
  "Only open contracts can be claimed": "只有待认领状态的项目才可以被认领。",
  "Only open or claimed contracts can receive additional claims": "只有待认领或审核中的项目可以继续接收认领。",
  "You have already claimed this contract": "你已经认领过这个项目了。",
  "Only the Creator can accept a worker claim": "只有甲方可以确认乙方认领。",
  "Only claimed contracts can accept a worker": "只有已被认领的项目才可以确认乙方。",
  "Only claimed contracts can accept an applicant": "只有已进入认领审核的项目才可以选择乙方。",
  "This contract does not have a pending worker claim": "当前项目没有待确认的乙方认领。",
  "Selected applicant was not found on this contract": "当前项目中没有找到这位认领方。",
  ...
  "Only open, claimed, or draft contracts can be cancelled": "只有待认领、已认领或草稿状态的项目可以取消。",
  ...
  "Only the Creator can approve milestones": "只有甲方可以批准里程碑。",
  "Contract must be active before approval": "合同必须处于进行中状态后才能批准。",
  "Only submitted milestones can be approved": "只有待审核的里程碑可以被批准。",
  ...
  "Only approved milestones can be released": "只有已批准的里程碑可以放款。",
  ...
  "Solana transaction failed": "这笔链上交易执行失败。",
